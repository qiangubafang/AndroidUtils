...
public abstract class StandOutWindow extends Service {
	...
	public static Intent getSendDataIntent(Context context,
		...
		return new Intent(context, toCls).putExtra("id", toId)
				...
				.putExtra("org.tcshare.logview.standout.data", data)
				.putExtra("org.tcshare.logview.standout.fromCls", fromCls)
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		if (intent != null) {
			...
			if (ACTION_SHOW.equals(action) || ACTION_RESTORE.equals(action)) {
				...
				Bundle data = intent.getBundleExtra("org.tcshare.logview.standout.data");
				int requestCode = intent.getIntExtra("requestCode", 0);
				@SuppressWarnings("unchecked")
				Class<? extends StandOutWindow> fromCls = (Class<? extends StandOutWindow>) intent
						.getSerializableExtra("org.tcshare.logview.standout.fromCls");
	...
	public Notification getPersistentNotification(int id) {
		...
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
			channelId = Notify.createNotificationChannel(this,"tcshare", "LogViewer");
		} else {
			channelId = "";
		}
