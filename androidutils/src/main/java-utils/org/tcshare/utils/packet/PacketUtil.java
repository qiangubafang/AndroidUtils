...
import org.tcshare.utils.bluetooth.IPacketUtil;
import org.tcshare.utils.bluetooth.ValidPacket;
 ...
 * 1	    2	 1	    1  	  1-N个字节	 2	    1
 */
public class PacketUtil implements IPacketUtil {
            ...
            if (b == header) {// 匹配第一个字节, 预测下一个头
                if (it.hasNext()) { // 忽略第二个字节，和第三个字节， （长度）
                    ...
                    if (it.hasNext()) {
                        ...
                        if (packetSize > maxLen || packetSize < FIX_PRE_LEN + FIX_SUF_LEN) { // 数据包太长或者太短
                            return -1; // 错误
                        }
    ...
    public synchronized void preparePacket(byte[] bytes, IPacketReadyCallBack cb) {
        for (byte b : bytes) {
            revBytesCache.add(b);
        }
