 ...
 */
package org.tcshare.poi.resolver;
...
public class TemplateResolver extends AbstractResolver {
    ...
    private static MyLogger logger = MyLogger.getLogger(TemplateResolver.class);
    ...
    public List<MetaTemplate> resolveDocument(XWPFDocument doc) {
        ...
        logger.info("Resolve the document end, resolve and create %d MetaTemplates.", metaTemplates.size());
    ...
    ElementTemplate parseTemplateFactory(String text, Object obj, XWPFRun run) {
        ...
        if (templatePattern.matcher(text).matches()) {
            ...
            if (null != elementTemplate) {
                logger.debug("Resolve where text: %s, and create %s for %s", text,
